<?php
namespace Opencart\Admin\Controller\Extension\AJAXaf\Module;
...
class AttributeFilter extends \Opencart\System\Engine\Controller {
    ...
    public function index(): void {
        ...
        $data['breadcrumbs'] = [
            ...
            [
                ...
                'href' => $this->url->link('extension/AJAXaf/module/attribute_filter', 'user_token=' . $this->session->data['user_token'] . ($module_id ? '&module_id=' . $module_id : ''))
            ]
        ];
        ...
        $data['save'] = $this->url->link('extension/AJAXaf/module/attribute_filter.save', 'user_token=' . $this->session->data['user_token'] . ($module_id ? '&module_id=' . $module_id : ''));
        $data['back'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module');
        $data['rebuild'] = $this->url->link('extension/AJAXaf/module/attribute_filter.rebuild', 'user_token=' . $this->session->data['user_token']);
        ...
        $data['attributes'] = $this->model_extension_AJAXaf_module_attribute_filter->getAttributes();

        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');
        $data['footer'] = $this->load->controller('common/footer');

        $this->response->setOutput($this->load->view('extension/AJAXaf/module/attribute_filter', $data));
    }
    ...
    public function save(): void {
        ...
        if ($this->validate()) {
            ...
            if ($module_id) {
                ...
                $json['module_id'] = $this->model_setting_module->addModule('AJAXaf.attribute_filter', $data);
            }

            if (!empty($data['auto_index'])) {
                $this->model_extension_AJAXaf_module_attribute_filter->registerEvents();
            } else {
                $this->model_extension_AJAXaf_module_attribute_filter->unregisterEvents();
            }

            $this->model_extension_AJAXaf_module_attribute_filter->rebuildIndex();
    ...
    public function install(): void {
        if ($this->user->hasPermission('modify', 'extension/AJAXaf/module/attribute_filter')) {
            $this->load->model('extension/AJAXaf/module/attribute_filter');
            $this->model_extension_AJAXaf_module_attribute_filter->install();
        }
    }
    ...
    public function uninstall(): void {
        if ($this->user->hasPermission('modify', 'extension/AJAXaf/module/attribute_filter')) {
            $this->load->model('extension/AJAXaf/module/attribute_filter');
            $this->model_extension_AJAXaf_module_attribute_filter->uninstall();
        }
    }
