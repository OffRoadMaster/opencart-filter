<?php
namespace Opencart\Catalog\Model\Extension\OpencartFilter\Module;
...
class AttributeFilter extends \Opencart\System\Engine\Model {
    ...
    public function getProductIds(int $category_id, int $language_id, array $filters): array {
        ...
        if ($category_id > 0) {
            $category_join = " INNER JOIN `" . DB_PREFIX . "product_to_category` ptc ON (ptc.product_id = p.product_id)";
            $category_where = " AND ptc.category_id = '" . (int)$category_id . "'";
        }

        if (!$filters) {
            $query = $this->db->query("SELECT DISTINCT p.product_id
                FROM `" . DB_PREFIX . "product` p" . $category_join . "
                WHERE p.status = '1'
                    AND p.date_available <= NOW()" . $category_where . "
        ...
        $joins = [];
        $index = 0;
        $index_category_id = max(0, $category_id);
        ...
        foreach ($filters as $attribute_id => $hashes) {
            ...
            $joins[] = "INNER JOIN `" . DB_PREFIX . "attribute_filter_index` " . $alias . " ON (" . $alias . ".product_id = p.product_id
                AND " . $alias . ".category_id = '" . (int)$index_category_id . "'
