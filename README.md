...
1. Build the package with `./tools/build_ocmod.sh` or zip only `install.json`, `README.md`, `admin/`, `catalog/`, and `ocmod/` so those files/folders are at the archive root. The ZIP file name should be `opencart_filter.ocmod.zip`, because OpenCart uses that name as the extension code and the module routes use `opencart_filter`.
